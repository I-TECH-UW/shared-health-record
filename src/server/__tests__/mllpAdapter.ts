...
describe("MllpAdapter#handleMessage", () => {
    ...
    let returnPromise: Promise<IBundle> = new Promise(res => { res }) 

    it("should handle ADT message", async () => {
        let msg = (await fs.readFile(path.join(__dirname, "../../__data__/sample_ADT.txt"))).toString()

        const saveAdtMessageSpy = jest.spyOn(Hl7Workflows, 'saveAdtMessage').mockReturnValue(returnPromise);
    ...
    it("should handle ORU message", async () => {
        ...
        const saveOruMessageSpy = jest.spyOn(Hl7Workflows, 'saveOruMessage').mockReturnValue(returnPromise);
