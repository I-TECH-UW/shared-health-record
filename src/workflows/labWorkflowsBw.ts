...
export class LabWorkflowsBw extends LabWorkflows {
  ...
  public static async createIpmsOrder(labBundle: R4.IBundle): Promise<R4.IBundle> {
    ...
    let mllpServer = new MllpServer(config.get('mllp:targetIp'), config.get('mllp:targetPort'), logger)

    let adtResult = await this.sendHl7(mllpServer, adtMessage)

    logger.info(`adt:\n${adtMessage}\nres:\n${adtResult}`)

    let obrMessage = await Hl7WorkflowsBw.getFhirTranslation(labBundle, "OBR.hbs");

    let obrResult = await this.sendHl7(mllpServer, obrMessage)
  ...
  private static sendHl7(mllpServer: MllpServer, msg: string) {
    return new Promise((resolve, reject) => {
      mllpServer.send(config.get('mllp:targetIp'), config.get('mllp:targetPort'), msg, (err: any, ackData: any) => {
