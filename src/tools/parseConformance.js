...
var ParseConformance = (function () {
  function ParseConformance(loadCached) {
    ...
    this.parsedStructureDefinitions = loadCached
      ? require("./profiles/types.json")
      : {};
    this.parsedValueSets = loadCached
      ? require("./profiles/valuesets.json")
      : {};
    this.codeSystems = [];
  }
  ParseConformance.prototype.isBaseProfile = function (url) {
    var urls = [
      ...
      "http://hl7.org/fhir/StructureDefinition/Condition (aka Problem)",
      ...
      "http://hl7.org/fhir/StructureDefinition/amingSystem",
      "http://hl7.org/fhir/StructureDefinition/utritionOrder",
  ...
  ParseConformance.prototype.loadCodeSystem = function (codeSystem) {
    ...
    var foundCodeSystem = _.find(this.codeSystems, function (nextCodeSystem) {
      return (
        nextCodeSystem.url === codeSystem.url ||
        nextCodeSystem.id === codeSystem.id
      );
    });
    if (!foundCodeSystem) {
  ...
  ParseConformance.prototype.parseStructureDefinition = function (
    structureDefinition
  ) {
    var uid = (
    ...
    this.parsedStructureDefinitions[structureDefinition.id || uid] =
      parsedStructureDefinition;
    var loadedStructureDefinition = _.find(
      this.structureDefinitions,
      function (sd) {
        return sd.url === structureDefinition.url;
      }
    );
      ...
      for (var x in structureDefinition.snapshot.element) {
        ...
        if (elementId.endsWith("[x]")) {
          ...
          for (var y in element.type) {
            ...
            choiceType =
              choiceType.substring(0, 1).toUpperCase() +
              choiceType.substring(1);
          ...
          if (
            ...
            this.populateBackboneElement(
              parsedStructureDefinition,
              structureDefinition.snapshot.element[x].id,
              structureDefinition
            );
          }
          if (
            ...
            if (element.type[0].profile) {
              ...
              let extension = newProperty._profile.substring(
                newProperty._profile.lastIndexOf("/") + 1
              );
              //console.log( this.parsedStructureDefinitions[extension] )
              newProperty._properties =
                this.parsedStructureDefinitions[extension]._properties;
            } else {
              newProperty._properties = [];
              this.populateExtension(
                parsedStructureDefinition,
                structureDefinition.snapshot.element[x].id,
                structureDefinition
              );
          ...
          if (includeValueSet) {
            _.each(includeValueSet.systems, function (includeSystem) {
              ...
              if (!foundSystem) {
                ...
                foundSystem.codes = foundSystem.codes.concat(
                  includeSystem.codes
                );
              }
            });
  ...
  ParseConformance.prototype.populateBackboneElement = function (
    resourceType,
    parentElementId,
    profile
  ) {
    var _loop_3 = function (y) {
      ...
      if (
        !backboneElementId.startsWith(parentElementId + ".") ||
        backboneElementId.split(".").length !==
          parentElementId.split(".").length + 1
      ) {
        return "continue";
      }
      backboneElementId = backboneElementId.substring(profile.type.length + 1);
      var parentElementIdSplit = parentElementId
        .substring(profile.type.length + 1)
        .split(".");
      var parentBackboneElement = null;
      var _loop_4 = function (j) {
        parentBackboneElement = _.find(
          !parentBackboneElement
            ? resourceType._properties
      ...
      if (parentBackboneElement) {
        if (!backboneElement.type) {
          ...
          parentBackboneElement._properties.push({
            _name: backboneElementId.substring(
              backboneElementId.lastIndexOf(".") + 1
            ),
            _type: type,
          ...
          for (var y_1 in backboneElement.type) {
            ...
            choiceType =
              ...
              choiceType.substring(1);
            var choiceElementId =
              backboneElement.id.substring(
                backboneElement.id.lastIndexOf(".") + 1,
                backboneElement.id.length - 3
              ) + choiceType;
            var newProperty = {
              _name: choiceElementId,
              _choice: backboneElement.id.substring(
                backboneElement.id.lastIndexOf(".") + 1
              ),
          ...
          this_2.populateValueSet(backboneElement, newProperty);
          if (
            backboneElement.type[0].code === "BackboneElement" ||
            backboneElement.type[0].code == "Element"
          ) {
            this_2.populateBackboneElement(
              resourceType,
              profile.snapshot.element[y].id,
              profile
            );
          }
        } else {
  ...
  ParseConformance.prototype.populateExtension = function (
    resourceType,
    parentElementId,
    profile
  ) {
