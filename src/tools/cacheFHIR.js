...
const deleteESDocument = (id, index, callback) => {
  ...
  axios({
    ...
    .catch((err) => {
      logger.error(
        "An error occured while trying to delete ES record with id " + id
      );
      logger.error(err);
      return callback();
    });
  ...
  fhirWrapper.getResource(
    ...
    (relationship) => {
      ...
      const sd =
        ...
        relationship.subject.reference.substring(
                    ...
                    async.eachSeries(
                      ...
                      (data, next) => {
                        ...
                        if (
                          ...
                          (!data.resource.link ||
                            (data.resource.link &&
                              Array.isArray(data.resource.link) &&
                              data.resource.link.length === 0) ||
                            (data.resource.link &&
                              !Array.isArray(data.resource.link)))
                        ) {
                          return next();
                        ...
                        const isGoldenRec =
                          data.resource &&
                          data.resource.meta &&
                          data.resource.meta.tag &&
                          data.resource.meta.tag.find((tag) => {
                            return (
                              ...
                              if (typeof value == "object") {
                                if (value.reference && fieldAutogenerated) {
                                  ...
                                  const referencedResource =
                                    await getResourceFromReference(
                                      value.reference
                                    );
