    ...
    return new Promise(resolve => {
        ...
        async.each(extension, (ext, nxtExt) => {
            ...
            (() => __awaiter(void 0, void 0, void 0, function* () {
                if (Array.isArray(value)) {
                    const val = yield getElementValFromExtension(value, element);
                    if (val) {
                        elementValue = val;
                    }
                    return nxtExt();
...
const getFields = (links, reportDetails) => {
    const details = reportDetails['http://ihris.org/fhir/StructureDefinition/iHRISReportElement'];
    const fields = [];
    ...
    function populateField(label) {
        ...
        if (keys.length !== 1) {
            logger.error('Something is wrong with the report relationship, cant determine data type for ' +
        ...
        createAnalyzer: callback => {
            ...
            axios.put(url, settings, auth).then(response => {
                if (response.status >= 200 && response.status <= 299) {
                    ...
                    logger.error('Something went wrong while creating analyzer into elasticsearch');
                    return callback(true);
                }
            }).catch(err => {
                if (err.response &&
                    ...
                    logger.info('Analyzer already exist into elasticsearch, not creating');
                    return callback(null);
                }
                else {
                    logger.error(err);
        ...
        createMapping: callback => {
            ...
            axios.put(url, mapping, auth).then(response => {
                ...
                if (err.response &&
                    err.response.status &&
                    (err.response.status === 400 || err.response.status === 403)) {
                    logger.info('Mappings already exist into elasticsearch, not creating');
                    return callback(null);
                }
                else {
                    logger.error('Something went wrong while adding mappings into elasticsearch');
...
const fhir2ES = ({ lastSync, patientsBundle }, callback) => {
    ...
    fhirWrapper.getResource({
        ...
        const sd = relationship.subject && relationship.subject.reference.substring(relationship.subject.reference.lastIndexOf('/'));
        structureDefinition(sd, (err, subject) => {
            ...
            for (const linkIndex1 in links) {
                ...
                for (const link2 of links) {
                    ...
                    if (flattenedLink2.linkTo === flattenedLink1.name &&
                        !flattenedLink2.linkElement.startsWith(flattenedLink2.resource + '.')) {
                ...
                createESIndex(reportDetails.name, IDFields, reportFields, err => {
                    ...
                    getImmediateLinks(orderedResources, links, () => {
                        async.eachSeries(orderedResources, (orderedResource, nxtResource) => {
                            ...
                            promise.then(() => {
                                logger.info('Writting resource data for resource ' +
                                ...
                                async.eachSeries(resourceData, (data, next) => {
                                    ...
                                    if (!data.resource || !data.resource.resourceType) {
                                        ...
                                        for (const query of queries) {
                                            ...
                                            const resourceValue = fhir.evaluate(data.resource, limitParameters);
                                        ...
                                        (() => __awaiter(void 0, void 0, void 0, function* () {
                                            for (const element of orderedResource['http://ihris.org/fhir/StructureDefinition/iHRISReportElement']) {
                                                ...
                                                const displayData = fhir.evaluate(data.resource, fieldName);
                                                let value;
                                                if ((!displayData ||
                                                    ...
                                                    value = yield getElementValFromExtension(data.resource.extension, fieldName);
                                                }
                                                ...
                                                if (typeof value == 'object') {
                                                    if (value.reference && fieldAutogenerated) {
                                                        ...
                                                        !fieldAutogenerated) {
                                                        const referencedResource = yield getResourceFromReference(value.reference);
...
//# sourceMappingURL=cacheFHIR.js.map