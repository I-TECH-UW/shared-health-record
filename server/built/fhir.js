...
module.exports = () => ({
     ...
     */
    getResource({ resource, extraPath = [], noCaching = false, url, id, query, count }, callback) {
        let statusCode = 500;
        let resourceData = {};
        resourceData.entry = [];
        if (!url) {
        ...
        async.whilst(callback1 => {
            ...
            request.get(options, (err, res, body) => {
                ...
                if (id && body || body.resourceType !== 'Bundle') {
                    ...
                    if (count) {
                        resourceData = Object.assign({}, body);
                    }
                    else {
    ...
    '$meta-delete'({ resourceParameters, resourceType, resourceID }) {
